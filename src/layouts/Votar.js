...
console.log("documento1 :: ----", idk);
   const  handleEvendSource = async ()=>{
    ...
    const response = await Axios.put("http://localhost:3002/updatvotantesfine/"+parseInt(idk)+"/"+parseInt(documento1))
    ...
    const eventFunction = async  ()=>{
      const response = await  Axios.get('http://localhost:3002/candidatosvista/'+parseInt(id))
    ...
    const handdleDefineVoto = ()=>{
        Swal.fire({
            title: `Estas seguro de que quieres votar por este candidato <img src="${candidato.map(id=>(id.imgUrl))}" />`,
            showDenyButton: true,
            ...
            if (result.isConfirmed) {
              ...
              Swal.fire('Se cancelo la solicitud debera iniciar nuevamente con su documento', '', 'warning')
            }
          })
